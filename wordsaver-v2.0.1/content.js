        ...
        chrome.runtime.sendMessage(message, function(response) {
            ...
            if (chrome.runtime.lastError) {
                ...
                if (errorMsg.includes("Extension context invalidated") || 
                    ...
                    handleExtensionReconnection(message, callback);
                    return;
                }
...
// 处理扩展重连
function handleExtensionReconnection(originalMessage, originalCallback) {
    // 显示用户友好的重连提示
    showReconnectionDialog(originalMessage, originalCallback);
}

// 显示重连对话框
function showReconnectionDialog(originalMessage, originalCallback) {
    // 移除可能存在的旧对话框
    const existingDialog = document.getElementById('wordsaver-reconnect-dialog');
    if (existingDialog) {
    ...
    dialog.id = 'wordsaver-reconnect-dialog';
    ...
    dialog.innerHTML = `
        <div style="color: #d32f2f; font-size: 24px; margin-bottom: 15px;">
            🔄
        </div>
        ...
        <p style="color: #666; margin: 0 0 20px 0; line-height: 1.5;">
            WordSaver 扩展需要重新连接才能继续工作。
            <br>请选择以下操作：
        </p>
        <div style="display: flex; gap: 10px; justify-content: center;">
            <button id="wordsaver-reload-page" style="
                background: #4285f4;
                color: white;
                border: none;
                padding: 10px 20px;
            ...
            <button id="wordsaver-retry-connection" style="
                background: #0f9d58;
                color: white;
                border: none;
                padding: 10px 20px;
            ...
            <button id="wordsaver-close-dialog" style="
                background: #666;
                color: white;
                border: none;
                padding: 10px 20px;
        ...
        <div style="margin-top: 15px; font-size: 12px; color: #999;">
            提示：使用 Ctrl+R (Windows) 或 Cmd+R (Mac) 快速刷新
        </div>
    `;
    ...
    // 绑定按钮事件
    document.getElementById('wordsaver-reload-page').addEventListener('click', () => {
        window.location.reload();
    });
    
    document.getElementById('wordsaver-retry-connection').addEventListener('click', () => {
        dialog.remove();
        retryConnection(originalMessage, originalCallback);
    });
    
    document.getElementById('wordsaver-close-dialog').addEventListener('click', () => {
        dialog.remove();
        if (originalCallback) {
            originalCallback({ error: '用户取消操作' });
        }
    });
    ...
    const handleKeyPress = (e) => {
        if (e.key === 'Enter') {
            document.getElementById('wordsaver-retry-connection').click();
        } else if (e.key === 'Escape') {
            document.getElementById('wordsaver-close-dialog').click();
        } else if ((e.ctrlKey || e.metaKey) && e.key === 'r') {
            e.preventDefault();
            document.getElementById('wordsaver-reload-page').click();
        }
    };
...
// 重试连接
function retryConnection(originalMessage, originalCallback) {
    console.log("正在重试扩展连接...");
    
    // 显示重试状态
    showNotification("正在重试连接...");
    ...
    setTimeout(() => {
        try {
            chrome.runtime.sendMessage({ action: "ping" }, function(response) {
                if (chrome.runtime.lastError) {
                    console.log("重试失败:", chrome.runtime.lastError.message);
                ...
                if (response && response.success) {
                    ...
                    if (originalMessage) {
                        setTimeout(() => {
                            trySendMessage(originalMessage, originalCallback);
                        }, 500);
                    }
                } else {
                    showNotification("重连失败，请刷新页面", "error");
